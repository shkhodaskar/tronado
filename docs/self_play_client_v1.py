...
def tron_client(env: TronGridClientEnvironment, username: str, trainer):
    ...
    while not terminal:
        ...
        action = Mapping[trainer.compute_actions(np.expand_dims(extractor.transform(obs),axis=0),rotate=player_num)[0][0]]
        
        # Uncomment for single agent
        #action = Mapping[trainer.compute_action(extractor.transform(obs))]
